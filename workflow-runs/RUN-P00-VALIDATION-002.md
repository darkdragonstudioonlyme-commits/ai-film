...
STATE: BLOCKED
BLOCK_REASON_CLASS: PRODUCT_QUALIFICATION_RECOVERY_GATE_MISMATCH
...
- TEST-DESIGN/TEST-REVIEW for the producer completed, but implementation of its required T14 entry matrix exposed TEST_GAP-P00-V03-QUALIFICATION-RECOVERY-002. Exact dev22 SITE RECONCILIATION_ONLY bypasses qualification because its C0 route makes active=false; all ten missing/invalid/mismatch recovery variants authorize although the approved matrix requires recovery rejection. Tooling implementation is paused and routes back to product WORKFLOW_REVIEW; native remains NOT_RUN.
