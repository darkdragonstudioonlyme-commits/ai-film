...
V02 is deliberately resumable. The disposable environment, exact-build deployment, isolation, restore-probed and sealed snapshots, exact authority draft, protected pending bundle, fail-closed authority intake and non-installing policy staging are complete technical prerequisites. The remaining block is external authority only. Preparation records, the watcher and the materializer cannot self-issue or install that authority.
