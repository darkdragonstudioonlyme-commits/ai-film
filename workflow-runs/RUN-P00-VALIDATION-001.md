...
- `aifilm-p00-v02-authority-watch.timer` is enabled/active every 5 minutes. The watcher only updates mode-600 local status evidence; observed systemd security exposure is `4.9 OK`, with no Internet socket families or capabilities. READY flags bind the SHA-256 of the current READY status; BLOCKED/error results remove stale flags. The watcher never starts LAB or mutates HKLM.
...
V02 is deliberately resumable. The disposable environment, exact-build deployment, isolation, restore-probed and sealed snapshots, exact authority draft, protected pending bundle, stale-safe fail-closed authority intake, non-installing policy staging and pre-V03 gate are complete technical prerequisites. The remaining block is external authority only. Preparation records, watcher, materializer, staging gate and installer cannot self-issue or install that authority without a valid external package and explicit reviewed commit action.