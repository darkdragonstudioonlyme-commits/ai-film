...
- Fail-closed authority intake: `validation/V02_AUTHORITY_INTAKE-P00-DEV21.md`. Validator SHA-256 `312a72ddc1a8551a85d93b275ea0ccfe84b87ca3ab9e575753d945c7b6f69667` checks exact candidate/content bindings, protected registration/attestations/recovery, all 86 suite rows, all 85 fixture specs, hash-addressed plans/role pins and pure `authority.authorize()` for every approved plan.
...
- `aifilm-p00-v02-authority-watch.timer` is enabled/active every 5 minutes. The watcher only updates mode-600 local status evidence; observed systemd security exposure is `4.9 OK`, with no Internet socket families or capabilities. Even a future valid intake only creates `READY_TO_ADVANCE.flag`; it does not start LAB or mutate HKLM.
...
V02 is deliberately resumable. The disposable environment, exact-build deployment, isolation, baseline/pristine snapshots, recovery proof, exact authority draft, protected pending bundle and fail-closed authority intake are now complete technical prerequisites. The remaining block is authority, not infrastructure: an external authority must place a valid protected approval package into the intake and independently approve registration/fixture/plan/suite records. Preparation records and the watcher cannot self-issue that authority.
