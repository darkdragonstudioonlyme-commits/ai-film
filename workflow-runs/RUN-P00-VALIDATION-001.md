...
- Production-like operations now include six enabled/active timers: integrity every 15 minutes, V02 authority watch every 5 minutes, health every 10 minutes, control backup every 24 hours, recovery verification after boot/every 6 hours, and Windows-host mirror every 2 hours. Latest health PASS includes verified-fresh local backup and host mirror plus last backup/recovery/mirror job `Result=success`.
- Latest control backup/mirror path is documented by `validation/WSL_PRODLIKE_OPERATIONS-P00-DEV21.md` and `validation/WSL_PRODLIKE_HOST_MIRROR-P00-DEV21.md`. The current archive contains 29 whitelisted control files only; local and NTFS mirror copies both pass hash/member verification, restore probe and secret/protected-domain scan, with retention 14 and freshness limit 30 hours. No native authority or protected identity is included.
