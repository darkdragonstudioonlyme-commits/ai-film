...
- Production-like runtime/operations evidence: `validation/WSL_PRODLIKE_RUNTIME-P00-DEV21.md`, `validation/WSL_PRODLIKE_OPERATIONS-P00-DEV21.md`, and `validation/WSL_PRODLIKE_HOST_MIRROR-P00-DEV21.md`. Exact dev21 app bytes remain immutable/read-only; integrity, health, backup, recovery and host-mirror verification are monitored without granting native authority.
- Production-like operations include six enabled/active timers: integrity every 15 minutes, V02 authority watch every 5 minutes, health every 10 minutes, control backup every 24 hours, recovery verification after boot/every 6 hours, and Windows-host mirror every 2 hours. Latest health PASS includes verified-fresh local backup and host mirror plus last supervised job results.
- All six oneshot services have bounded execution through separate systemd timeout drop-ins: authority watch 2 minutes; integrity, health and host mirror 5 minutes; backup and recovery 10 minutes; stop timeout 30 seconds. Base service bytes remain stable, including the integrity-service SHA stored by the runtime manifest.
- Current local/NTFS control archive contains 35 whitelisted files only, including all six timeout drop-ins. Local and NTFS copies pass hash/member verification, restore probe and secret/protected-domain scan, with retention 14 and freshness limit 30 hours. No native authority or protected identity is included.
...
V02 is deliberately resumable. The disposable environment, exact-build deployment, isolation, restore-probed and sealed snapshots, exact authority draft, protected pending bundle, stale-safe fail-closed authority intake, non-installing native-policy staging and pre-V03 gate are complete technical prerequisites. The remaining block is external authority only. Production-like monitoring/backup/recovery/host-mirror controls cannot self-issue or install that authority.