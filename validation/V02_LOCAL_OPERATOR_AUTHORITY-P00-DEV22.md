...
This is intentionally **not external authority**. The owner requires all execution to remain on the same Windows/WSL trust domain. After reviewed activation, an Ed25519 private key will be stored locally under an owner-only WSL path and used to sign the exact approval envelope. The signature proves local key possession and signed-byte integrity; it does not prove an independent controller or out-of-band provenance.
