...
HARDENING_STATUS: CANDIDATE_NOT_DEPLOYED
...
`aifilm-p00-v02-authority-watch.timer` is enabled and active every 5 minutes. Its oneshot service has no Internet socket families or capabilities and an observed `systemd-analyze security` exposure score of `4.9 OK`. It only refreshes mode-600 local status evidence. Forensic review found that the deployed watcher correctly clears READY on normal BLOCKED/error results but could exit during non-JSON status parsing before its cleanup branch. Candidate SHA `2a25159b...` fixes this by deleting READY before every evaluation and recreating it only from the current successful READY evidence; three persistent adversarial cases cover malformed output, BLOCKED and READY. It still does not start LAB or advance the canonical run automatically.
