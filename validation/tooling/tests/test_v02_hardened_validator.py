...
def prepare_tool_copy(base):
    ...
    raw=(json.dumps(synthetic,sort_keys=True,separators=(',',':'))+'\n').encode();(tool/'v02-local-identity-context.json').write_bytes(raw)
    lp=tool/'v02_local_identity.py';text=lp.read_text();text=re.sub(r"EXPECTED_CONTEXT_SHA256='[0-9a-f]{64}'",f"EXPECTED_CONTEXT_SHA256='{hashlib.sha256(raw).hexdigest()}'",text);lp.write_text(text)
    return tool

def run_case(preflight,root):
    p=subprocess.run([str(preflight),'--inbox',str(root),'--json'],text=True,capture_output=True)
    data=json.loads((p.stdout.strip().splitlines() or ['{}'])[-1]);assert data.get('staging_unchanged') is True;assert data.get('native_execution_started') is False
    return p.returncode,data

def write_env(root,obj):
    (root/'objects').mkdir(parents=True,exist_ok=True);(root/'approval-envelope.json').write_text(json.dumps(obj,sort_keys=True,separators=(',',':')))
def expect(label,preflight,root,rc,reason,status):
    got_rc,data=run_case(preflight,root);assert(got_rc,data.get('reason'),data.get('status'))==(rc,reason,status),(label,got_rc,data);print('PASS',label,reason)
...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        r=cases/'flags-only';write_env(r,approved);expect('flags-only',preflight,r,11,'LOCAL_AUTHORITY_ANCHOR_PENDING','INVALID')
        ...
        r=cases/'fake-refs';write_env(r,fake);expect('fake-refs',preflight,r,11,'LOCAL_AUTHORITY_ANCHOR_PENDING','INVALID')
        (r/'approval-envelope.sig.json').write_text(json.dumps({'schema_version':1,'algorithm':'ED25519','key_id':'LOCAL-KEY','payload_sha256':'0'*64,'signature_b64':'AA=='},sort_keys=True,separators=(',',':')))
        expect('local-signature-with-pending-anchor',preflight,r,11,'LOCAL_AUTHORITY_ANCHOR_PENDING','INVALID')
