...
OWNER_ACTION: open http://localhost:8765/ and score 12 VoxCPM2 samples + blind motion_A/motion_B on the 0–8 scale, then export/copy JSON.
PAID_RESOURCE: source=projects/slice01/runtime/gpu_session.json | state=RUNNING idle at owner-review gate | provider_billed=USD 9.710121/USD 60 | stop recommended if review will pause.
...
COST_LEDGER: measured execution total USD 0.455474 including rejected Paris v1; provider lifetime bill USD 9.710121 including GPU + disk. Budget guard follows provider spend.
NEXT: ingest the complete owner review JSON, unblind motion only after all 14 scores exist, then promote only scored media into the first multi-shot M3/M4 quality benchmark.
