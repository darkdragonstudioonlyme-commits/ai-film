...
class RunPodA40LiveAuthorityTests(unittest.TestCase):
    ...
    def test_cost_policy_and_bootstrap_estimate_enforce_60_cap(self):
        ...
        self.assertEqual(len(COST_LEDGER["entries"]),25)
        ...
        self.assertAlmostEqual(sum(float(row["amount_usd"]) for row in COST_LEDGER["entries"]),0.455474,places=6)
        ok=budget_decision(COST_LEDGER,budget_usd=60.0,proposed_charge_usd=59.5445)
        self.assertTrue(ok["allowed"])
        blocked=budget_decision(COST_LEDGER,budget_usd=60.0,proposed_charge_usd=59.5446)
