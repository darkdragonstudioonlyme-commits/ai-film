...
class SessionRunner:
    ...
    def _fresh(self, interface, plan, *, step=None, held=False):
        ...
        if held:
            ...
            # Latest immutable references, not a cached authorization, accompany
            # subsequent intents. The guard operator population is independently
            # checked by the native driver on every refresh.
    ...
    def execute(self, interface: str, plan: dict) -> dict:
        ...
        with c.acquire(auth):
            ...
            current = (progress[len(progress)-1]['material_after']
                       if progress else semantic['before'])
            # No NOOP based on a marker. All committed steps still require a
            # fresh identity/profile/resource check and actual final assertions.
            ...
            for index in range(len(progress), len(semantic['operations'])):
                ...
                try:
                    ...
                    # Observe-side native probes must have persisted their own
                    # witness before resume too; use the final current witness.
                    require(terminal['native_witness'] == c.fence['native'],
                            19, 'NATIVE_WITNESS_MISMATCH')
                    c.terminal(terminal)
                    current = terminal['material_after']
                    progress[index] = terminal
                except BaseException as error:
                    if c.fence is not None:
                        # A journal write failure during terminal may already
                        # have committed; preserve, never overwrite a TERMINAL
                        # with a fictional successful second outcome.
                        if c.fence['state'] != 'TERMINAL':
                            try:
                                c.uncertain(error.reason if isinstance(error, P00Error)
                                            else 'CONTROLLER_INTERRUPTED')
                            except BaseException:
                                pass  # original failure + durable INTENT remain
                        if hasattr(self.driver,'record_failure'):
                            try:self.driver.record_failure(plan,error,c)
                            except BaseException:
                                pass  # capture failure cannot override or erase original uncertainty
                    raise
            ...
            require(type(final_assertions) is dict and bool(final_assertions),
                    19, 'FINAL_ASSERTIONS_MISSING')
            # A pure postcondition observer result is recorded, not promoted.
    ...
    def _live_noop(self,interface,plan,fresh,progress):
        ...
        witness={**witness,'execution_phase':'LIVE_REVALIDATION',
                 'original_progress_digest':progress_digest(progress)}
        reservation=budget_check(fresh.observed.get('remaining_budgets',plan['semantic']['budgets']),
