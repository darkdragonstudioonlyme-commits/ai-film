...
class Coordinator:
    ...
    def intent(self,action:str,reservations:list,witness:dict):
        ...
        # Write-ahead fence first; a journal append failure must not erase it.
    ...
    def native_child_started(self,native_witness:dict):
        """Additional command in the same approved action; persist before resume."""
        self._require_fence()
    ...
    def terminal(self,observation:dict):
        ...
        # Journal commit precedes clear. Failure retains a fence for explicit reconciliation.
    ...
    def safe_pause(self, observation: dict):
        """Release an observed quiescent, revoked run without committing its step.

        The caller must be renewed original-fence reconciliation, not apply. A
        pause/cancel never kills a process, retries an action, or proves success.
        A failed CLEAR leaves SAFE_PAUSE durable for exact-release reconciliation.
        """
        self._require_fence()
        require(self.admission.purpose == 'RECONCILIATION_ONLY', 12,
                'RECONCILIATION_SCOPE')
        require(type(observation) is dict and observation.get('action') == self.fence['action'],
                19, 'PAUSE_ACTION_MISMATCH')
        require(observation.get('disposition') in ('PAUSE', 'CANCEL'), 10, 'PAUSE_DISPOSITION')
        require(observation.get('no_pending_writer') is True
        ...
        require(observation.get('native_witness') == self.fence.get('native'),
    ...
    def close(self):
        # Releasing a process primitive NEVER implies clearing its durable fence.
        if self.held:
            ...
            if not pending and hasattr(self.storage, 'has_pending_reads'):
                ...
                except BaseException: pending = True  # journal uncertainty cannot release native admission
