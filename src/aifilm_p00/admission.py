...
class Coordinator:
    ...
    def safe_pause(self, observation: dict):
        ...
        require(observation.get('no_pending_writer') is True
                ...
                and observation.get('postconditions_observed') is False, 21, 'SAFE_PAUSE_NOT_PROVEN')
        from .codec import hash_value
        for key in ('evidence_digest', 'revocation_ref', 'request_digest'): hash_value(observation.get(key))
    ...
    def _persist(self,kind): self.storage.write_fence(self.fence); self._event(kind)
    ...
    def close(self):
        if self.held:
            ...
            if pending and hasattr(self.guard,'defer_until_process_exit'): self.guard.defer_until_process_exit()
            else: self.guard.release()
            self.held=False
