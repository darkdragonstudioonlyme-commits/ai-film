...
def draft(root,selection_ref,capture_digest):
    """Read exact protected capture, derive and journal an UNAPPROVED proposal.

    No active source refresh is hidden in dry-run. Freshness/current identity is
    checked again by execution entry. Missing snapshot or bindings do not cause
    a guest launch, dependency install, auto approval, or default-distro lookup.
    """
    ...
    # Exact metadata permission is already the project's C0 output contract.
    ...
    try:
        ...
        # Bind syntax and desired transition without treating it as approval.
        # A bounded journal-only output avoids revealing native paths/SIDs/raw
        # captured data through ordinary stdout.
