"""E00-01..17 immutable group/field catalog and stage requiredness.
This module evaluates supplied observations; it does not create native facts.
"""
from dataclasses import dataclass
from copy import deepcopy
from .codec import digest,fields,hash_value,instant,token
...
def cell(value,source_ref,observed_at,*,source_kind,status='OBSERVED',reason=None):
    return {'status':status,'actual':deepcopy(value),'source_ref':source_ref,'observed_at':observed_at,'source_kind':source_kind,'reason':reason}
...
def group_check(eid,body,stage,*,downstream=False):
    ...
    require(body['schema_version']==1 and body['evidence_id']==eid and type(body['fields']) is dict and set(body['fields'])==set(CATALOG[eid]),15,'CATALOG_FIELD_SET')
    ...
    return {'complete_for_stage':not missing,'missing_fields':missing,'stage':stage.name,'body_digest':digest(body),'host_ready':False}
...
def new_body(eid,values,*,at,source_ref,source_kind,missing_status='UNAVAILABLE',missing_reason='NOT_COLLECTED'):
    ...
    return {'schema_version':1,'evidence_id':eid,'fields':{name:cell(values[name],source_ref,at,source_kind=source_kind) if name in values else pending(missing_status,missing_reason,at) for name in CATALOG[eid]},'evaluations':[]}
...
def evaluated(body,name,passed,expected_ref,reason=None):
    ...
    result['evaluations'].append({'name':name,'status':'PASS' if passed else 'FAIL','input_digest':digest(body['fields']),'expected_ref':expected_ref,'reason':reason})
    return result
