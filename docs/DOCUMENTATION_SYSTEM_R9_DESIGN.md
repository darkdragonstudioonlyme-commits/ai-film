...
- activation status and activation evidence;
- effectiveness status and evidence;
- successor when ineffective/superseded;
- measurement trigger/next review condition.
...
R9 deliberately automates **detection, reconciliation and routing**, not unreviewed policy mutation.
...
→ checker derives backlog / stale activation / measurement debt
→ workflow-health trigger if debt or ineffectiveness exists
...
- derive learning backlog and ineffective-count metrics;
...
3. review/activation/effectiveness transitions are internally valid;
4. activation targets that are already active releases are not left silently pending;
5. `PROJECT_STATE` learning backlog/ineffective counts equal derived register counts;
6. ineffective or measurement-due entries surface workflow-health debt.
...
- `LEARNING-SOURCE-VISIBILITY-001` — active and effective; formal handoffs now distinguish artifact-only/partial visibility from exact identity;
