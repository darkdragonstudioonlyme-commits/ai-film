...
from film.flux2_klein_live import validate_model_dir
...
PROFILE_PATH = ROOT / "model-evaluations/slice01/flux2_casting_profile.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the fixed FLUX.2 klein casting smoke on an authorized A40."
    )
    parser.add_argument("--execute", action="store_true")
    parser.add_argument(
        "--model-dir",
        default="/workspace/models/flux2-klein-4b",
    )
    parser.add_argument(
        "--out-root",
        default="/workspace/artifacts/flux2-casting-smoke",
    )
    parser.add_argument(
        "--evidence-out",
        default="/workspace/runs/flux2-casting-smoke/evidence.json",
    )
    ...
    if not torch.cuda.is_available():
        raise Flux2CastingError("CUDA is not available")
