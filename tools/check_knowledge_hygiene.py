...
mem=(ROOT/'PROJECT_MEMORY.md').read_text(encoding='utf-8');lines=mem.splitlines();entries=len(re.findall(r'\bMEM-\d{8}-\d{3}\b',mem))
if len(lines)>250: errors.append(f'memory-lines:{len(lines)}>250')
if entries>40: errors.append(f'memory-entry-refs:{entries}>40')
version_agnostic=['README.md','CHAT_HANDOFF.md','WORKFLOW_ROUTER.md','EXECUTION_LANES.md','DOCUMENTATION_MAP.md','GIT_WORKFLOW.md','PROJECT_ROADMAP.md','TEST_STRATEGY.md','SELF_LEARNING_SYSTEM.md','KNOWLEDGE_LIFECYCLE.md'];pat=re.compile(r'\b(?:0\.1\.0\.)?dev\d+\b',re.I)
for name in version_agnostic:
    if pat.search((ROOT/name).read_text(encoding='utf-8')): errors.append('mutable-version-in-policy:'+name)
...
snap=json.loads((ROOT/'evidence/SERVER_ENVIRONMENT_SNAPSHOT.json').read_text(encoding='utf-8'));fp=snap.get('environment_fingerprint_sha256')
