...
def main() -> int:
    ap=argparse.ArgumentParser(description="Plan or execute one exact FLUX.2 klein 4B casting qualification job on the authorized RunPod A40.")
    ap.add_argument("--job-id",required=True)
    ap.add_argument("--max-runtime-sec",type=float,default=1800.0)
    ap.add_argument("--execute",action="store_true")
    ap.add_argument("--model-dir")
    ap.add_argument("--out-root",default="/workspace/runs/flux2-klein-live")
    args=ap.parse_args()
    ...
    qual=prepare_flux2_qualification(
        job=job,
        matrix=matrix,
