...
if not REPO.is_dir() or not (WS/'implement').is_dir() or not (WS/'review').is_dir(): print('RUNTIME_STATE_CHECK_SKIPPED prepared WSL workspace unavailable');raise SystemExit(0)
...
def grab(pattern):
    ...
    return m.group(1)
base=grab(r'BASE_COMMIT: ([0-9a-f]{40})');review_target=grab(r'TARGET_COMMIT: ([0-9a-f]{40})');planned=grab(r'PLANNED_VERSION: (0\.1\.0\.dev\d+)')
...
porcelain=run('git','-C',str(WS/'implement'),'status','--porcelain',strip=False).splitlines();actual_dirty={line[3:] for line in porcelain if len(line)>=4}
block=re.search(r'DIRTY_FILES:\n((?:\s+- .+\n)+)',state);expected_dirty={line.strip()[2:] for line in block.group(1).splitlines()} if block else set()
...
pkg=WS/'artifacts/IMPL-P00-001_IMPLEMENTATION_PACKAGE_V17.zip';expected='130f43c1b54ce00c19a894c61dfa0edfc4218a434ba4d1f060ef815cfaff951e'
if not pkg.is_file() or hashlib.sha256(pkg.read_bytes()).hexdigest()!=expected: errors.append('durable-package-drift')
helper=WS/'test.sh';expected_helper='#!/usr/bin/env bash\nset -euo pipefail\nexec /usr/bin/python3 /home/dragon/ai-film-dev/repo/tools/run_test_workflow.py implement\n'
if 'ACTIVE_SYSTEM: V2_REVIEWED' in state:
    if not helper.is_file() or helper.read_text(encoding='utf-8')!=expected_helper: errors.append('workspace-test-helper-drift')
if errors: print('STATE_DRIFT');print('\n'.join(errors));raise SystemExit(1)
