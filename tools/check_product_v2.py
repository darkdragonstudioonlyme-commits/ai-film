...
REQUIRED_PRODUCT_FILES = [
    ...
    "tools/run_image_keyframe_live.py",
...
def main() -> int:
    ...
    if wan_profile is None:
        ...
        if wan_profile.get("quality_status")!="AWAITING_OWNER_SCORING" or wan_profile.get("production_acceptance") is not False:
    ...
    if len(entries)!=len(expected_cost_ids) or set(by_cost_id)!=expected_cost_ids:
        ...
        if abs(float(by_cost_id["wan22-quality-wan22-sc01-sh04-flux2-ref-v1-pass"].get("amount_usd",0))-0.037649)>1e-9:
    ...
    keyframe_spec=json.loads((root/"model-evaluations/slice01/video/keyframe_probe_sc01_sh04.json").read_text(encoding="utf-8"))
    key_jobs=keyframe_spec.get("jobs",[])
    if keyframe_spec.get("status")!="READY_NOT_EXECUTED" or keyframe_spec.get("shot_id")!="sc01_sh04" or len(key_jobs)!=2:
        errors.append("keyframe-probe-shape")
    if {row.get("model_id") for row in key_jobs}!={"flux2-klein-4b","z-image"}:
        errors.append("keyframe-probe-model-balance")
    if any(row.get("width")!=704 or row.get("height")!=1280 or row.get("style")!="photoreal" for row in key_jobs):
        errors.append("keyframe-probe-portrait-contract")
    if keyframe_spec.get("selection_authorized") is not False or keyframe_spec.get("quality_acceptance") is not False:
        errors.append("keyframe-probe-authority-boundary")
    key_batch=json.loads((root/"model-evaluations/slice01/batches/sc01_sh04_portrait_keyframes_20260926.json").read_text(encoding="utf-8"))
    if key_batch.get("batch_id")!="sc01-sh04-portrait-keyframes-20260926" or len(key_batch.get("jobs",[]))!=2:
        errors.append("keyframe-batch-shape")
    if {row.get("adapter") for row in key_batch.get("jobs",[])}!={"image-keyframe-live"}:
        errors.append("keyframe-batch-adapter")
