...
strategy=(ROOT/'TEST_STRATEGY.md').read_text(encoding='utf-8');nextwork=(ROOT/'NEXT_WORK_ITEM.md').read_text(encoding='utf-8')
...
def field(name):
    m=re.search(r'^\s*'+re.escape(name)+r':\s*(.+?)\s*$',nextwork,re.M);return m.group(1).strip().strip('"\'') if m else None
allowed={'OWNER_REQUIREMENTS','REVIEWED_CONTRACTS','ACCEPTANCE_MATRIX','REVIEW_FINDINGS','VALIDATION_SPEC','COMPOSITE_APPROVED_AUTHORITIES'};auth=field('ORACLE_AUTHORITY_CLASS')
if auth not in allowed: errors.append('oracle-authority-class:'+str(auth))
change=field('TEST_CHANGE_CLASS');allowed_change={'NONE','APPROVED_BEHAVIOR_CHANGE','TEST_DEFECT','HARNESS_DEFECT','ENVIRONMENT_DEFECT','IMPLEMENTATION_DEFECT','DESIGN_GAP'}
if change not in allowed_change: errors.append('test-change-class:'+str(change))
source=(field('ORACLE_SOURCE') or '').lower();forbidden=['current code','code behavior','implementation output','source implementation','implementation behavior','whatever the code']
