...
forensic_promotion=section_field('FORENSIC_HARDENING','PROMOTION_FINALIZATION_EVIDENCE')
forensic_authority=section_field('FORENSIC_HARDENING','AUTHORITY_REFERENCE_EVIDENCE')
forensic_ci_credential=section_field('FORENSIC_HARDENING','CI_CREDENTIAL_ISOLATION_EVIDENCE')

def forensic_promotion_parity_drift(dst):
    jp=dst/f'AI_FILM_PROJECT_STATE_V{state_version}.json'
    data=json.loads(jp.read_text(encoding='utf-8'))
    data['forensic_hardening']['promotion_finalization_evidence']=forensic_authority
    jp.write_text(json.dumps(data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
require('forensic_promotion_finalization_parity_drift',run_case(forensic_promotion_parity_drift),False,'forensic-evidence-parity:PROMOTION_FINALIZATION_EVIDENCE')

def forensic_authority_parity_drift(dst):
    jp=dst/f'AI_FILM_PROJECT_STATE_V{state_version}.json'
    data=json.loads(jp.read_text(encoding='utf-8'))
    data['forensic_hardening']['authority_reference_evidence']=forensic_ci_credential
    jp.write_text(json.dumps(data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
require('forensic_authority_reference_parity_drift',run_case(forensic_authority_parity_drift),False,'forensic-evidence-parity:AUTHORITY_REFERENCE_EVIDENCE')

def forensic_ci_credential_parity_drift(dst):
    jp=dst/f'AI_FILM_PROJECT_STATE_V{state_version}.json'
    data=json.loads(jp.read_text(encoding='utf-8'))
    data['forensic_hardening']['ci_credential_isolation_evidence']=forensic_authority
    jp.write_text(json.dumps(data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
require('forensic_ci_credential_evidence_parity_drift',run_case(forensic_ci_credential_parity_drift),False,'forensic-evidence-parity:CI_CREDENTIAL_ISOLATION_EVIDENCE')

def forensic_promotion_missing_target(dst):
    missing='workflow-health/DOES-NOT-EXIST-V52.md'
    p=dst/'PROJECT_STATE.md'; text=p.read_text(encoding='utf-8')
    old=f'  PROMOTION_FINALIZATION_EVIDENCE: {forensic_promotion}'
    if old not in text: raise RuntimeError('forensic promotion line missing')
    text=text.replace(old,f'  PROMOTION_FINALIZATION_EVIDENCE: {missing}',1)
    p.write_text(text,encoding='utf-8')
    jp=dst/f'AI_FILM_PROJECT_STATE_V{state_version}.json'
    data=json.loads(jp.read_text(encoding='utf-8'))
    data['forensic_hardening']['promotion_finalization_evidence']=missing
    jp.write_text(json.dumps(data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
...
def section_field(section,name):
    m=re.search(rf'^{re.escape(section)}:\s*$([\s\S]*?)(?=^[A-Z][A-Z0-9_]*:\s*$|\Z)',STATE,re.M)
    if not m: raise RuntimeError(f'missing state section {section}')
    fm=re.search(rf'^\s+{re.escape(name)}:\s*(.*?)\s*
def ordinal(value):
    m=re.search(r'-(\d+)$',value)
    if not m: raise RuntimeError(f'no ordinal in {value}')
    return int(m.group(1))
...
,m.group(1),re.M)
    if not fm: raise RuntimeError(f'missing state field {section}.{name}')
